...
struct HelloWorldHandler {
  ...
  void operator()(RequestContext &rawCtx, const TestContext &requestContext,
                  const Request &in, TestContext &replyContext, Reply &out) {
    using namespace std::chrono;
    const auto now = system_clock::now();
    const auto sinceEpoch = system_clock::to_time_t(now);
    out.name = fmt::format(
        "Hello World! The local time is: {}",
        std::put_time(std::localtime(&sinceEpoch), "%Y-%m-%d %H:%M:%S"));
    out.byteArray = in.name; // doesn't really make sense atm
    out.byteReturnType = 42;

    out.timingCtx = opencmw::TimingCtx(
        3, {}, {}, {}, duration_cast<microseconds>(now.time_since_epoch()));
    ...
    replyContext.ctx = opencmw::TimingCtx(
        3, {}, {}, {}, duration_cast<microseconds>(now.time_since_epoch()));
    replyContext.contentType = requestContext.contentType;
    replyContext.testFilter =
        fmt::format("HelloWorld - reply topic = {}", requestContext.testFilter);
  }
};
...
class ImageServiceWorker
    : public Worker<serviceName, TestContext, Empty, BinaryData, Meta...> {
...
public:
  ...
  explicit ImageServiceWorker(const BrokerType &broker,
    ...
    for (const auto &path : fs.iterate_directory("/testImages")) {
      if (path.is_file()) {
        const auto file =
            fs.open(fmt::format("testImages/{}", path.filename()));
        imageData.push_back(
    ...
    notifyThread = std::jthread([this, updateInterval] {
      while (!shutdownRequested) {
        ...
        TestContext context;
        // TODO ideally we could send this notification to any subscription
        // independent of their contentType
        ...
        // TODO the subscription via REST has a leading slash, so this "/" is
        // necessary for it to match, check if that can be avoided
        super_t::notify("/", context, reply);
      }
    });

    super_t::setCallback([this](RequestContext &rawCtx, const TestContext &,
                                const Empty &, TestContext &, BinaryData &out) {
...
int main() {
  ...
  Worker<"helloWorld", TestContext, Request, Reply,
         description<"A friendly service saying hello">>
      helloWorldWorker(primaryBroker, HelloWorldHandler());
  ImageServiceWorker<"testImage", description<"Returns an image">> imageWorker(
      primaryBroker, std::chrono::seconds(10));

  std::jthread helloWorldThread(
      [&helloWorldWorker] { helloWorldWorker.run(); });

  std::jthread imageThread([&imageWorker] { imageWorker.run(); });
